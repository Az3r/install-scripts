...
mkdir -p "$HOME/.config/i3" && ln -vsf "$DOTFILE_DIR/i3.conf" "$HOME/.config/i3/config"
mkdir -p "$HOME/.config/polybar" && ln -vsf -t "$HOME/.config/polybar" "$DOTFILE_DIR/polybar.conf"
mkdir -p "$HOME/.config/polybar" && ln -vsf "$DOTFILE_DIR/polybarlaunch.sh" "$HOME/.config/polybar/launch.sh"
...
# blocked because these require user's password
chmod +x "$DOTFILE_DIR/polybarlaunch.sh"
